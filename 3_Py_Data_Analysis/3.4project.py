...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    # create a list of tuples: playerIDs and computed formula statistic
    player_tup_list = []
    for line_dict in statistics:
        player_id = line_dict[info["playerid"]]
        player_stat = formula(info, line_dict)
        player_tup = (player_id, player_stat)
        player_tup_list.append(player_tup)
    # sort tuple list by computed statistic
    player_tup_list.sort(key=lambda pair: pair[1], reverse = True)
    # return only top numplayers requested
    return player_tup_list[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    csv_output = read_csv_as_nested_dict(info["masterfile"],
                                         info["playerid"],
                                         info["separator"],
                                         info["quote"])
    my_list = []
    for my_player_id, stat in top_ids_and_stats:
        if my_player_id in csv_output:
            first_name = csv_output[my_player_id][info['firstname']]
            last_name = csv_output[my_player_id][info['lastname']]
            my_name = first_name + " " + last_name
            my_str = "{0:0.3f} --- {1}".format(stat, my_name)
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    top_ids_and_stats = top_player_ids(info, filtered_stats, formula, numplayers)
    chosen_list = lookup_player_names(info, top_ids_and_stats)
    return chosen_list
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    nested_agg_dic = aggregate_by_player_id(statistics,
                                            info["playerid"],
                                            info['battingfields'])

    # convert nested_agg_dic to list_agg_dic
    list_agg_dic = []
    for dic in nested_agg_dic:
        list_agg_dic.append(nested_agg_dic[dic])

    top_ids_and_stats = top_player_ids(info, list_agg_dic, formula, numplayers)
